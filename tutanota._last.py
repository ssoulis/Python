...
class TutanotaPlugin(plugin_base.BotPluginBase):
    ...
    def registration_action(self):
        ...
        #self.sleep(60*10)
        #r = requests.get("https://mail.tutanota.com/signup", allow_redirects=False)
        self.load_website("https://mail.tutanota.com/signup")
        #self.take_screenshot("fvalue.png")
        page=self.get_default_page()
        self.sleep(5)
        self.run_script_and_return(\
            'document.getElementsByClassName("button-content flex items-center login plr-button justify-center")[0].click()')
        self.sleep(5)
        self.run_script_and_return(\
     ...
     'document.getElementsByClassName("input")[3]',page,captcha_text)
             ...
             return bot_core.StandardActionInstanceResult(self.get_user_profile_id(),
          False)
        recovery=self.run_script_and_return('document.getElementsByClassName("text-break")[1].innerText')
        self.run_script_and_return(\
            'document.getElementsByClassName("limit-width noselect bg-transparent button-height full-width")[0].click()')
        ...
        self.sleep(30)
        self.store_credentials("tutanota.com", selected_username, selected_password, phone_number = recovery)
