...
class ProtonMailPlugin(plugin_base.BotPluginBase):
    ...
    def login(self, username, password, page):
        self.load_website('https://mail.protonmail.com/login')
        self.sleep(5)
        self.type('document.getElementById("username")', page, username)
        self.type('document.getElementById("password")', page, password)
        self.run_script_and_return('document.getElementById("login_btn").click()')
        self.sleep(5)

    #just login and stay here for a little while
    def maintenance_action(self):
        profile_id = self.get_user_profile_id()
        username, password, email, phone = self.get_credentials("protonmail.com")
       ...
       # self.sleep(5)
        ...
        if in_captcha == False:
           ...
           # 'https://sms-activate.ru/stubs/handler_api.php?api_key=4b6520de7602181de853201229dA7c6d&action=getNumber&service=dp&forward=$forward&operator=$operator&ref=$ref&country=12&phoneException=$phoneException')
            ...
            self.sleep(5)
            res = requests.Response('https://sms-activate.ru/stubs/handler_api.php?api_key=4b6520de7602181de853201229dA7c6d&action=getNumber&service=dp&forward=$forward&operator=$operator&ref=$ref&country=12&phoneException=$phoneException')
            data = res.split(":")
            my_id = data[1]
            number = data[2]
            self.sleep(5)
            ...
            self.type('document.getElementsByName("smsVerification")',page,number)
            self.sleep(2)
            ...
            #ama ginei resend
            while code == "STATUS_WAIT_RESEND" :
                self.find_and_click('Send new code','BUTTON')
                self.sleep(5)
            #change activation status 
                status = requests.get(\
                'https://sms-activate.ru/stubs/handler_api.php?api_key=4b6520de7602181de853201229dA7c6d&action=setStatus&status=6&id=my_id&forward=$forward')
                print(status)
                print("waiting for new sms...")                                    
                self.sleep(10)
            if code == "STATUS_CANCEL" :
                status = requests.get(\
                'https://sms-activate.ru/stubs/handler_api.php?api_key=4b6520de7602181de853201229dA7c6d&action=setStatus&status=8&id=my_id&forward=$forward')
                print(status)
                ...
                status = requests.get(\
                'https://sms-activate.ru/stubs/handler_api.php?api_key=4b6520de7602181de853201229dA7c6d&action=setStatus&status=1&id=my_id&forward=$forward')
                print(status)
                print("sms to the number sent successfully")
            self.type('document.getElementById("codeValue")',page,code)
            self.sleep(5)
